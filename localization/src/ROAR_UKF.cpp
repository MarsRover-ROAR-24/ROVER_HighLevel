...
void UKF::predict_measurement(double dt, double lon0, double lat0)
{
    ...
    ***/
    ...
    std::tie(zp, Pz) = unscented_transform(Z_sigma,
        sigma_points.Wm,
        sigma_points.Wc,
        R);
}
...
void UKF::update(Eigen::MatrixXd z_measurement)
{
    ...
    for (int i = 0; i < sigma_points.num_sigma_points; i++)
    {
		T = T + sigma_points.Wc(i) * (X_sigma.row(i) - x_hat.transpose()) * (Z_sigma.row(i) - zp.transpose());
	}

	// Compute Kalman gain
	Eigen::MatrixXd K = T * Pz.inverse();

	// Update state estimate
	x_hat = x_hat + K * (z_measurement - zp);
