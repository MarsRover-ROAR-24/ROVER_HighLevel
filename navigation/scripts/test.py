...
class Control:
    ...
    def __init__(self):
        ...
        self.path_subscriber = rospy.Subscriber('/selected_path', Path, self.tuple_list_callback)  
    ...
    def tuple_list_callback(self, msg):
        # Extract the waypoints from the Path message
        self.waypoints = [(pose.pose.position.x, pose.pose.position.y) for pose in msg.poses]
        if self.waypoints:
            self.x_goal_point = self.waypoints[0][0]
            self.y_goal_point = self.waypoints[0][1]
