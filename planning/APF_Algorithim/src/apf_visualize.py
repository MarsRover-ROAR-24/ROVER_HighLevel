...
def create_force_arrow_marker(pose, force):
    arrow = Marker()
    ...
    yaw = math.atan2(force.get_unit_vector().y, force.get_unit_vector().x)
...
def draw_force_map(pose_force_pair_list, marker_pub):
    marker_array = MarkerArray()
    count = 1
    for pose_force_pair in pose_force_pair_list:
        force_marker = create_force_arrow_marker(pose_force_pair[0], pose_force_pair[1])
...
Goal_Pos = [0.5,4.5]
Obs_Pos = [2.5,2.5]
APF_Param = [8,100,1.4] #[K_att,K_rep,q_star]
#######################################################################
#######################################################################
# APF Equations
x_rob = symbols('x_rob')
